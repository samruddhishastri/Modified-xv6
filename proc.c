    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        freevm(p->pgdir);
          ...
          for(int i=0; i < q_num_proc[p->queue]; i++){
            if(queue[p->queue][i] -> pid == p->pid){
              rem = i;
              proc_found = 1;
              break;
            }
          ...
          if(proc_found){
            for(int i = rem; i < q_num_proc[p->queue]-1; i++)
              queue[p->queue][i] = queue[p->queue][i+1]; 
            q_num_proc[p->queue]--;
          }
				#endif 
        p->pid = 0;
...
{ 
    ...
    struct proc *p;
    cprintf("\n pid \t\t priority \t state \t\t\t rtime \t\t n_run\n\n");
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->pid!=0){
        cprintf(" %d \t\t %d \t\t %s \t\t %d \t\t %d\n", p->pid, p->priority, states[p->state], p->rtime, p->n_run);
      }
    }
  #endif
  #ifdef FCFS
    ...
    struct proc *p;
    cprintf("\n pid \t\t state \t\t\t rtime \t\t wtime \t\t n_run \t\t cur_q \t\t q0 \t\t q1 \t\t q2 \t\t q3 \t\t q4\n\n");
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->pid!=0){
