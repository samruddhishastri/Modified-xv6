 ...
 169:	31 f6                	xor    %esi,%esi
{
 ...
 188:	50                   	push   %eax
 189:	6a 00                	push   $0x0
 18b:	e8 33 01 00 00       	call   2c3 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 1f1:	e8 f5 00 00 00       	call   2eb <open>
  if(fd < 0)
 ...
 205:	50                   	push   %eax
 206:	e8 f8 00 00 00       	call   303 <fstat>
  close(fd);
 20b:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 20e:	89 c6                	mov    %eax,%esi
  close(fd);
 210:	e8 be 00 00 00       	call   2d3 <close>
  return r;
 ...
 36c:	89 d1                	mov    %edx,%ecx
{
 ...
 3de:	56                   	push   %esi
 3df:	57                   	push   %edi
 3e0:	e8 e6 fe ff ff       	call   2cb <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
 ...
 459:	e8 6d fe ff ff       	call   2cb <write>
 ...
 4b9:	e8 0d fe ff ff       	call   2cb <write>
        putc(fd, c);
      } else {
 ...
 4cd:	e8 f9 fd ff ff       	call   2cb <write>
  for(i = 0; fmt[i]; i++){
 ...
 541:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 544:	e8 82 fd ff ff       	call   2cb <write>
        ap++;
 ...
 594:	eb 1a                	jmp    5b0 <printf+0x1a0>
          s = "(null)";
 596:	bb 8f 07 00 00       	mov    $0x78f,%ebx
        while(*s != 0){
 ...
 5bb:	57                   	push   %edi
 5bc:	53                   	push   %ebx
 5bd:	e8 09 fd ff ff       	call   2cb <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
 ...
 6db:	e8 53 fc ff ff       	call   333 <sbrk>
  if(p == (char*)-1)
